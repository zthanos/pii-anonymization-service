"""gRPC server with V3 worker pool implementation."""
...
from .grpc_servicer_v3 import StructuredAnonymizerV3ServicerImpl
...
async def serve_v3(
    ...
    """
    Start gRPC server with V1, V2, and V3 APIs.
    
    V3 uses worker pool pattern for improved performance.
    
    Args:
        structured_tokenizer: Tokenizer instance
        port: Port to listen on
        max_workers: Number of gRPC worker threads
        max_concurrent: Max concurrent requests (V1 only)
        batch_size: Batch size for V1 streaming
        worker_pool_size: Number of workers in the pool (V3)
        worker_queue_size: Size of work queue (V3)
    """
    ...
    # Add V3 servicer (worker pool - optimized)
    v3_servicer = StructuredAnonymizerV3ServicerImpl(
        structured_tokenizer=structured_tokenizer,
    ...
    # Start worker pool
    await v3_servicer.start()
    
    # Register V3 servicer as V2 API (same proto, different implementation)
    ...
    print(f"  - V2 API: pii.v2.StructuredAnonymizerV2 (batch + worker pool)")
    print(f"  - Worker Pool: {worker_pool_size} workers, queue size {worker_queue_size}")
